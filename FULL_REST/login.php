...
<html lang="es" dir="ltr">
  ...
  <body style="background: #d0dee5; overflow-x: hidden">
    ...
    <div class="container" style="height: 30vh">
      <div class="row" style="text-align: center; height: 100%; place-content: center;">
        ...
        <p style="color: black; font-size: 1.3rem" class="pt-5">Inicio de sesión | <span style="font-weight: bold">Usuario</span></p>
      </div>
    </div>
    ...
    <div class="container" style="height: 50vh">
      ...
      <div class="row" id="div_formulario_reenvio" style="display: none; height: 100%; place-content: center;">


        <div class="row" style="justify-content: center;align-content: center; height: 15%">
          <div class="col-lg-5 div_input">
            <input type="text" class="form-control" name="" value="" placeholder="Correo" style="height: 45px ; background: rgb(255,255,255, 0.7); text-align: center;" id="correo_reenvio">
          </div>
        </div>
        ...
        <div class="row" style="text-align: center; height: auto">
          <p style="color: red; font-size: 1.2rem; visibility: hidden; margin: 0" id="error_reenvio_invalido">Correo o contraseña invalidos</p>
          <p style="color: red; font-size: 1.2rem; display: none; margin: 0" id="error_boot">Este correo ya se encuentra verificado</p>
          <p style="color: red; font-size: 1.2rem; display: none; margin: 0" id="error_snake">Ocurrió un error, intente de nuevo más tarde</p>
          <p style="color: red; font-size: 1.2rem; display: none; margin: 0" id="error_feather">Este correo no se encuentra registrado</p>
        </div>

        <div class="row" style="justify-content: center;align-content: center; height: 15%">
          <div class="col-md-2 div_boton">
            <button type="button" name="button" class="boton_login" id="boton_reenviar">Reenviar</button>
          </div>
        </div>

        <div class="row" style="text-align: center;align-content: center; height: 10%">
          <p style="color: #0d6efd; font-size: 1.2rem; margin: 0; cursor: pointer" class="regresar">← Regresar</p>
        </div>


      </div>

      <div class="row" id="div_gracias" style="display: none">

        <p style="color: black; font-size: 2rem; text-align: center">Correo de verificación enviado!</p>
  ...
  $(".boton_login").click(function() {
    ...
    if(temporal.indexOf(false) == -1){
      ...
      $.ajax({
        ...
        url:   'REST/api/login.php', //archivo que recibe la peticion
        ...
        success: function(data) {
          ...
          if (data == "code_ok") {
            window.location.href = "sistema-bsystems.php";
          }
  ...
  $("#boton_reenviar").click(function() {

    var correo = $("#correo_reenvio").val();
    ...
    if(correo != ""){
      if(/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/.test(correo)){
        ...
        $("#error_reenvio_invalido").css("visibility", "visible");
      }
    }else{
      $("#error_reenvio_invalido").css("visibility", "visible");
    }
    ...
    if(temporal != false){
      //
      $.ajax({
        data:  {"correo": correo}, //datos que se envian a traves de ajax
        url:   'REST/api/reenvio.php', //archivo que recibe la peticion
        type:  'post', //método de envio
        success: function(data) {
          var data = data.replace(/(\r\n|\n|\r)/gm, "");
  ...
  $("#reenviar_enlace").click(function() {
    $("#div_formulario").hide();
    $("#div_formulario_reenvio").show();
  });
  ...
  $("#correo_reenvio").click(function() {
    $("#error_reenvio_invalido").css("visibility", "hidden");
    $("#error_boot").hide();
    $("#error_snake").hide();
    $("#error_feather").hide();
  });
